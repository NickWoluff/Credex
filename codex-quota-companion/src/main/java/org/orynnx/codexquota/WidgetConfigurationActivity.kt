...
abstract class BaseWidgetConfigurationActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val options = buildList {
            val state = QuotaRepository.current(this@BaseWidgetConfigurationActivity)
            if (QuotaRepository.signedIn(this@BaseWidgetConfigurationActivity) || state.hasWeekly || state.hasFiveHour) {
            ...
            StandardBalanceRepository.forSurface(this@BaseWidgetConfigurationActivity, BalanceSurface.LAUNCHER, Int.MAX_VALUE)
                .forEach { service -> add(service.id to "${service.name} · ${platformBrand(service.authMode).displayName}") }
        }
