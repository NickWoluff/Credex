...
`content://org.orynnx.codexquota/quota/assistant`

Build the import ZIP with `python -X utf8 demo/codex-quota-rear-card/build_card.py` (or `.uild_card.ps1` on Windows), then import it in OuterView. Authorize the companion app before enabling the card.
...
It does not run Codex on Android and cannot access the OAuth token. Balance rows
follow the Assistant display selection and order configured in the companion app.
